n = 5;                 %order of constellation
m = 2^n;                %number of symbols
distance = 1;           %minimum distance between symbols
t = 10;                %number of symbols sent

[refConst,SymbolCoordinates2Transpose,SymbolData, inphase, quadr] = HQAM(distance, n);
...
for i=1:t
    x = randi(m);
    rxSig = awgn(refConst(x),25,sigpower);
...
%demodData = genqamdemod(rxSig,refConst);
%[numErrors,ser] = symerr(data,demodData);


function [refConst,SymbolCoordinates2Transpose,SymbolData, inphase, quadr] = HQAM(distance, n)
    m = 2^n;                        %number of symbols
    c = 2;
    ...
    if rem(n,2)==0 && n>=4
            ...
            i = 1;                                                                           %matrix index
            offset = 0;  
            h_o = sqrt(m)*distance/2 - distance/4;                                                  %horizontal offset
            v_o = (sqrt(m)/2 - 1)*sqrt(3)*distance/2 + sqrt(3)*distance/4;          %vertical offset
            %for l=-a/2:a/2-1
            for j=0:a-1
                    if(j~=0 && rem(j,2)==0)
                            offset = offset+1;    
